// Test final du système corrigé - Version définitive
const fs = require('fs');

console.log('=== TEST FINAL SYSTÈME CORRIGÉ ===\n');

// Importer le générateur corrigé
const { generateUltimateSiteFixed } = require('./src/lib/ultimateTemplateFixed.js');
...
testLeads.forEach((lead, index) => {
  ...
  try {
    const html = generateUltimateSiteFixed(lead);
