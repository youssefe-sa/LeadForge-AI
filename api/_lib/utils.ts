  ...
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8">
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .container { width: 95%; max-width: 750px; margin: 20px auto; background: white; padding: 20px 25px; border-radius: 4px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); position: relative; overflow: hidden; }
      .print-btn { position: absolute; top: 10px; right: 25px; background: #D4500A; color: white; padding: 6px 12px; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 10px; z-index: 10; }
      @media print { .print-btn { display: none; } .container { box-shadow: none; margin: 0; width: 100%; padding: 15px; } body { background: white; } }
      .header { display: flex; justify-content: space-between; border-bottom: 2px solid #edf2f7; padding-bottom: 10px; margin-bottom: 10px; align-items: flex-start; }
      .client-info { display: flex; justify-content: space-between; margin-bottom: 15px; background: #f8fafc; padding: 12px; border-radius: 6px; border: 1px solid #edf2f7; font-size: 12px; }
      ...
      table.pro-table td { padding: 3px 10px; border-bottom: 1px solid #f7fafc; font-size: 11px; line-height: 1.3; vertical-align: top; }
    ...
    <div class="container">
      ...
      <div class="header">
        ...
        <div style="text-align:right; flex-shrink: 0;">
          <h2 style="margin:0; font-size:20px; font-weight:900;">DEVIS</h2>
          <p style="margin:0; color:#718096; font-size:10px;">N° ${devisNumber}<br>Date : ${today}</p>
        </div>
      </div>
      ...
      <div class="price-box">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            <h3 style="margin:0; font-size:18px;">INVESTISSEMENT : ${price} €</h3>
            <p style="margin:2px 0 0 0; font-size:10px; opacity:0.8;">La création de votre site web est payée une seule fois.</p>
          </div>
      ...
      <div style="margin-top:15px; padding-top:10px; border-top:1px solid #edf2f7; display:flex; justify-content:space-between; align-items:center;">
        <div>
          <p style="font-size:10px; color:#718096; margin:0;">Signature Prestataire</p>
  ...
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8">
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .container { width: 95%; max-width: 750px; margin: 40px auto; background: white; padding: 40px; border-radius: 4px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); position: relative; }
