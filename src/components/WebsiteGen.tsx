...
Retourne UNIQUEMENT le HTML complet.`;
  ...
  // ── GENERATE CONTENT FOR ULTIMATE V2 TEMPLATE ──
  const generateContent = async (lead: Lead) => {
    ...
    const content: SC = {
      ...
      services: getServices().map((name, i) => ({ 
        ...
        features: ['Qualité garantie', 'Intervention rapide', 'Tarifs transparents']
      })),
  ...
  const generateSite = async (lead: Lead) => {
    ...
    try {
      ...
      try {
        ...
        const content = await generateContent(lead);
        const emergencyHtml = generateUltimateSiteV2(lead, content);
        updateProgress({ step: '☁️ Hébergement Cloud (Storage)...' });
...
<body>
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      // Re-générer instantanément le HTML localement. Le Design Ultimate V2 est préservé !
